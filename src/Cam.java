...
/**
 * Created by Shigemasa on 2017/03/25.
...
public class Cam{
    ...
    public static int numset(char dest[], int addValue, int addPoint) {
        ...
        if(dest.length <addPoint||addPoint<0){
            System.out.println("break first");return -1;}
        int length = getDigit(addValue);
        if(addValue<0){
            mflag=1;
            length+=1;
        }
        ...
        if ((dest.length-addPoint)<length){
            System.out.println("break second ");return -1;}
    ...
    public static void numset(char dest[], long addValue, int addPoint) {
        long srcmod;
        int length = getDigit(addValue);long mod = addValue;
        for (int i = 0; i < length; i++) {
            srcmod = mod / POWER_10L[length - 1 - i];
            dest[addPoint + i] = NUM[(int)(srcmod % 10)];
        }
    }
...
//        System.out.println("dLength = " + dLength);
    ...
    public static void numset(char dest[],int addValue){
        numset(dest,addValue,0);
    }
    public static void charset(char dest[],char addValue){
        charset(dest,addValue,0);
    }
    public static void charset(char dest[],char addValue,int addPoint){
        if(dest.length <addPoint||addPoint<0||(dest.length-addPoint)<1){
            System.out.println("break first");return;}
